async function cargarFactura() {
    ...
    // SUPABASE_URL y SUPABASE_ANON_KEY se cargan desde js/supabase-config.js
    ...
    try {
        ...
        // (el id se codifica: viene de la URL y no debe poder alterar el
        // filtro de PostgREST ni romper la petición)
        const idSeguro = encodeQueryValue(id);
        const [resFactura, resDetalles] = await Promise.all([
            fetch(
                `${SUPABASE_URL}/rest/v1/facturas?id_factura=eq.${idSeguro}&select=*`,
                { headers }
            ),
            fetch(
                `${SUPABASE_URL}/rest/v1/factura_detalles?id_factura=eq.${idSeguro}&select=*`,
                { headers }
            )
        ]);
        ...
        if (factura.referencia) {
            referenciaInfo.innerHTML = `
                ...
                ${escapeHtml(factura.referencia)}
                ${factura.banco ? `- ${escapeHtml(factura.banco)}` : ""}
            `;
        } else {
            referenciaInfo.innerHTML = "";
        }
        ...
        if (detalles.length === 0) {
            ...
            detalles.forEach(producto => {
                ...
                tr.innerHTML = `
                    <td>${escapeHtml(producto.cantidad || 0)}</td>
