...
document.querySelectorAll(".ledger-tabs button").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    if (btn.dataset.panel === "recientes") buscarFacturasHoy();
  });
});
...
function renderFacturasRecientes(facturas) {
  ...
  // Se guarda aparte de __facturasActuales para no pisar los datos
  // del panel de Ventas cuando se abre el detalle de una factura de hoy.
...
async function verDetalle(idFactura) {
  const factura = (window.__facturasActuales || [])
    .concat(window.__facturasRecientes || [])
    .find((f) => f.id_factura === idFactura);
